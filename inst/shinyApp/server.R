...
# for ppca ----------------------------------------------------------------
  ...
  sample_size_PPCA_spectral_data <- reactive({ # Read data
    ...
    # main_columns <- main_columns[main_columns!=input$sample_size_PPCA_label_slider] # remove label
  ...
  # This reactive output contains the dataset and display the dataset in table format
  output$sample_size_PPCA_spectral_data_table <- renderDataTable({ # Render Table
    if(is.null(sample_size_PPCA_spectral_data())){return ()}
    # head(main_data(), input$lab_slider)
    sample_size_PPCA_spectral_data()
  }, options = list(pageLength=10), escape = FALSE)
  ...
  ## Pilot PPCA Covariates Data
  sample_size_PPCA_covariates_data <- reactive({ # Read data
    if(input$sample_size_PPCA_cov_slider[1]==input$sample_size_PPCA_cov_slider[2]) {
      cov_data <- as.data.frame(sample_size_PPCA_data()[, input$sample_size_PPCA_cov_slider[1]])
      colnames(cov_data) <- colnames(sample_size_PPCA_data())[input$sample_size_PPCA_cov_slider[1]]
      cov_data

    } else {
      sample_size_PPCA_data()[, input$sample_size_PPCA_cov_slider[1]:input$sample_size_PPCA_cov_slider[2]]
    }
  })

  output$sample_size_PPCA_covariates_data_table <- renderDataTable({ # Render Table
    if(is.null(sample_size_PPCA_covariates_data())){return ()}
    validate(
      need(nrow(sample_size_PPCA_covariates_data()) == nrow(sample_size_PPCA_data()),
           "Please make sure equal rows in spectral and covariates data")
    )
    sample_size_PPCA_covariates_data()
  }, options = list(pageLength=10), escape = FALSE)



  # Sample Size PPCA parameter button
  ...
  # Sample Size PPCA data button
  observeEvent(input$sample_size_PPCA_pilot_data_button,
               isolate({
                 updateTabsetPanel(session, "sample_size_PPCA_tab",
  ...
  # Sample Size Return Parameter Button
  ...
  # Sample Size Pilot Check
  observeEvent(input$sample_size_PPCA_pilot_check,
               if(input$sample_size_PPCA_pilot_check == TRUE) {
                 updateTabsetPanel(session, "sample_size_PPCA_tab",
                                   selected = "sample_size_PPCA_pilot_data")
               }
  )
  ...
  # Check if covariates data are selected
  sample_size_cov_column_check <- reactive(input$sample_size_PPCA_cov_slider[2])
  sample_size_pilot_check <- reactive(input$sample_size_PPCA_pilot_check)

  observe(
    if(sample_size_cov_column_check() == 0 & sample_size_pilot_check() == TRUE) {
      updateCheckboxInput(session, "sample_size_PPCA_covariates_check", value = FALSE)
      disable('sample_size_PPCA_covariates_check')
    }
    else {
      enable('sample_size_PPCA_covariates_check')
    }
  )

  # Check if covariates are ticked for modelling
  sample_size_cov_check <- reactive(input$sample_size_PPCA_covariates_check)

  # Reactive expression to create data frame of all input values
  estimation_parameter_PPCA_report <- reactive({

    data.frame(
      Name = c("Spectral Bins",
               "Proportion of significant bins (m)",
               "Target FDR"),
      Value = as.character(c(input$sample_size_PPCA_n_bins,
                             input$sample_size_PPCA_prop_sig_bins,
                             input$sample_size_PPCA_target_fdr)),
      stringsAsFactors = FALSE)

  })

  # Submit Sample Size PPCA Parameter Button
  observeEvent(input$sample_size_PPCA_submit_button,
               isolate({

                 # Show the submitted values in an HTML table
                 output$estimation_parameter_PPCA_report <- renderTable({
                   input$sample_size_PPCA_submit_button
                   isolate(estimation_parameter_PPCA_report())
                 })
                 ...
                 if(cov_check()) { # include covariates
                   ...
                   output$sample_size_PPCA_estimate <- renderPlot({
                     isolate(
                       if(input$sample_size_PPCA_pilot_check) {
                         withProgress({metsize(pilot=sample_size_PPCA_spectral_data(),
