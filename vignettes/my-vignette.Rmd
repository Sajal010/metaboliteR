...
```{r, cache=TRUE}
ppca = PPCA(data = UrineSpectra[[1]],q_min = 1, q_max = 10, B=100, eps = 0.1)
```
...
ppcca = PPCA(data = UrineSpectra[[1]], covariates_data = weights, q_min = 1, q_max = 10, B =100)
```
...
The selected g and q for the brain spectra data set were 4 and 7 respectively. Using the folliwong commands we can assess estimates of the model parameters.
